...
@Service
public class UpdateFirstJob implements ExpireRecoveryJob{

    private final ReservationsRepository reservationsRepository;
    private final EventsRepository eventsRepository;
    private final SeatsRepository seatsRepository;
    public UpdateFirstJob(ReservationsRepository reservationsRepository, EventsRepository eventsRepository, SeatsRepository seatsRepository) {
    ...
    public List<UUID> runOnce(int batchSize, String workerId) {
        ...
        int i = reservationsRepository.claimExpiredPending(workerId, batchSize);

        if( i == 0 )
            throw new RuntimeException("failed to claim");

        List<ReservationRefs> refs = reservationsRepository.findClaimedExpiringIds(workerId, batchSize);
